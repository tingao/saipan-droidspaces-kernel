...
| Host tuning | charge band 75-80 %, wakeup source `saipan-awake`, CPU ceilings re-asserted every 60 s |
...
**Write order matters.** Writing `upper_limit` resets `lower_limit` to `-1` in this driver, so
upper must go first and lower second. Verified on-device:
...
Leaving a phone permanently at 100 % on a charger is the fastest way to wear the cell; the
75-80 % band keeps it near the flat part of the Li-ion curve. `CHARGE_UPPER=-1` restores stock
behaviour.
...
cmdline. I measured all three segments and kept the stock one; the overclock is a trap and
[CPU-CLOCK.md](CPU-CLOCK.md) has the numbers.
...
### 3.5 The container must not be able to sleep the phone
